...
`sudo bash -x -c 'id && umask 0077 && mkdir -p /root/.ssh && ssh-keygen -f id_rsa -N "" && echo StrictHostKeyChecking no >> /root/.ssh/config && echo UserKnownHostsFile /dev/null >> /root/.ssh/config && cat /root/.ssh/id_rsa.pub >> /root/.ssh/authorized_keys'`;
